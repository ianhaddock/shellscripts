...
#  Requires flatten script on Homer, and Shake and opendcp on the render nodes. #
#										  #
...
cat << EndOfUseage
  ...
  using the flatten script on Homer and Shake and OpenDCP on 
  the render nodes. 
...
cin2tif() {
   ...
   if [ $COUNT -eq $(( ($TOTALNODES * $SHAKEPERNODE) - 1)) ]; then
      ...
      ssh -f -n root@192.168.1.${NODESAVAIL[$(($COUNT/$SHAKEPERNODE))]} "sh -c '/usr/nreal/shake-linux-v4.10.0830/bin/shake $NODE_PWD/${FOLDERNAME}${FLATFOLDER}/${FILENAME}.#######.${EXTENSION} -cpus 8 $DELOGC -reorder rgb -bytes 2 -fo $NODE_PWD/${FOLDERNAME}${CINOUTPUTFOLDER}/${FILENAME}.#######.tif Auto rgb -t $FFOA-$LFOA >> ${NODE_PWD}/$LOGNAME 2>&1 &'"

      if [ $? -ne 0 ]; then
	echo "Could not ssh to 192.168.1.${NODESAVAIL[$(($COUNT/$SHAKEPERNODE))]}. Bailing out."
	exit 1; 
      fi 
...
tif2j2k() {
  ...
  until [ $COUNT -eq $TOTALNODES ]; do
   ...
   if [ $COUNT -eq $((TOTALNODES - 1)) ]; then
     ...
     ssh -f -n root@192.168.1.${NODESAVAIL[$COUNT]} "sh -c 'sleep 2; /usr/bin/opendcp_j2k $P3 -z -l 1 -s $FFOA -d $LFOA -i $NODE_PWD/${FOLDERNAME}${TIFINPUTFOLDER}/ -o $NODE_PWD/${FOLDERNAME}${TIFOUTPUTFOLDER}/ >> ${NODE_PWD}/$LOGNAME 2>&1 &'"

      if [ $? -ne 0 ]; then
        echo "Could not ssh to 192.168.1.${NODESAVAIL[$COUNT]}. Bailing out."
        exit 1;
      fi 
...
j2k2mxfxml() {
   ...
   ssh -f -n root@192.168.1.${NODESAVAIL[$COUNT]} "sh -c 'cd ${NODE_PWD}/${DCPNEWNAME}${MXFOUTPUTFOLDER}/ ; /usr/bin/opendcp_mxf -i ${NODE_PWD}/${FOLDERNAME}${TIFOUTPUTFOLDER} -o ${NODE_PWD}/${DCPNEWNAME}${MXFOUTPUTFOLDER}/${DCPNEWNAME}.mxf ; sleep 3 ; opendcp_xml --reel ${NODE_PWD}/${DCPNEWNAME}${MXFOUTPUTFOLDER}/${DCPNEWNAME}.mxf --title ${DCPNEWNAME} ' "
...
# checks which nodes are available to use
   ...
   if [[ $(DMRrenderNodeControl.py --getFarmState 1-24 | grep -i flush-hosts ) ]] ; then
      ...
      ssh -f -n root@192.168.3.121 "sh -c 'mysqladmin flush-hosts'"
      printf " Checking nodes, "
   fi
